...
def monobook_fix_html(doc, page_url):
    ...
    doc = remove_tag(
        doc,
        '<div class="generated-sidebar portlet" id="p-For_Editors"',
        "</div>",
        "<div",
    )
    doc = remove_tag(
        doc, '<div class="generated-sidebar portlet" id="p-ToDo"', "</div>", "<div"
    )
    ...
    if config.special_mode:
        ...
        doc = re.sub(r'<div id="f-poweredbyico">[\s\S]+?(<ul id="f-list">)', r"\1", doc)
    ...
    doc = re.sub(
        r"Audacity Development Manual</title>", r"Audacity Manual</title>", doc
    )
...
def pos_html_transform(doc, url, filename):
    ...
    if config.special_mode:
        ...
        doc = re.sub(
            r'<script type="text/javascript" src="https://[\s\S]+?</script>', r"", doc
        )
...
def find_unused_filename(filename, exists=os.path.exists):
    ...
    i = 1
    while True:
        numbered = os.path.splitext(tail)[0] + str(i) + os.path.splitext(tail)[1]
...
def parse_css(doc, url):
    ...
    for item in L:
        ...
        if config.no_images and any(
            u.strip().lower().endswith(suffix)
            for suffix in (".jpg", ".gif", ".png", ".ico")
        ):
            item.url = ""
            continue
...
def should_follow(url):
    ...
    if (url.count("&") >= 1 or url.count(";") > 0) and not any(
        x in url for x in (".css", "gen=css")
    ):
        if config.debug:
    ...
    if config.no_images and any(
        url.strip().lower().endswith(suffix)
        for suffix in (".jpg", ".gif", ".png", ".ico")
    ):
        if config.debug:
...
def run(out=sys.stdout):
    ...
    if urlparse.urlparse(config.rooturl)[1].lower().endswith("wikipedia.org"):
        ...
        out.write(
            "Instead, install the Wikipedia database locally and use mw2html on\n"
        )
        out.write(
            "your local installation.  See the Mediawiki site for more information.\n"
        )
        sys.exit(1)
    ...
    deploy_file(
        "AudacityLogo.png",
        r"alphamanual.audacityteam.org/m/resources/assets/AudacityLogo.png",
    )
    deploy_file(
        "303.css", r"alphamanual.audacityteam.org/m/skins/monobook/main.css/303.css"
    )
    deploy_file(
        "headbg.jpg", r"alphamanual.audacityteam.org/m/skins/monobook/headbg.jpg"
    )
    deploy_file("audio.png", r"alphamanual.audacityteam.org/m/skins/monobook/audio.png")
    deploy_file(
        "bullet.gif", r"alphamanual.audacityteam.org/m/skins/monobook/bullet.gif"
    )
    deploy_file(
        "external.png", r"alphamanual.audacityteam.org/m/skins/monobook/external.png"
    )
    deploy_file(
        "external_rtl.png",
        r"alphamanual.audacityteam.org/m/skins/monobook/external_rtl.png",
    )
