...
pdfFile = PdfPages(
    "patest_suggested_vs_streaminfo_latency_"
    + str(sampleRate)
    + pdfFilenameSuffix
    + ".pdf"
...
def setFigureTitleAndAxisLabels(framesPerBufferString):
    title(
        "PortAudio suggested (requested) vs. resulting (reported) stream latency\n"
...
individualPlotFramesPerBufferValues = [
    0,
    64,
    128,
    256,
    512,
]  # output separate plots for these
...
for framesPerBuffer in compositeTestFramesPerBufferValues:
    commandString = (
        testExeName
        + " "
        + str(inputDeviceIndex)
        + " "
        + str(outputDeviceIndex)
        + " "
        + str(sampleRate)
        + " "
        + str(framesPerBuffer)
    ...
    if framesPerBuffer in individualPlotFramesPerBufferValues:  # individual plots
        ...
        plot(
            d.suggestedLatency,
            d.halfDuplexOutputLatency,
            label="Half-duplex output latency",
        )
        plot(
            d.suggestedLatency,
            d.halfDuplexInputLatency,
            label="Half-duplex input latency",
        )
        plot(
            d.suggestedLatency,
            d.fullDuplexOutputLatency,
            label="Full-duplex output latency",
        )
        plot(
            d.suggestedLatency,
            d.fullDuplexInputLatency,
            label="Full-duplex input latency",
        )
        ...
        setFigureTitleAndAxisLabels(
            "user frames per buffer: " + str(framesPerBufferText)
        )
        setDisplayRangeSeconds(2.2)
        pdfFile.savefig()
        setDisplayRangeSeconds(0.1)
        setFigureTitleAndAxisLabels(
            "user frames per buffer: " + str(framesPerBufferText) + " (detail)"
        )
        pdfFile.savefig()
