...
def find_program(self, filename, **kw):
    ...
    if not ret:
        self.fatal(
            kw.get("errmsg", "") or "Could not find the program %s" % ",".join(filename)
        )
    if var:
        self.env[var] = ret
    return ret
...
def find_perl_program(self, filename, path_list=[], var=None, environ=None, exts=""):
    try:
        app = self.find_program(
            filename, path_list=path_list, var=var, environ=environ, exts=exts
        )
    except Exception:
