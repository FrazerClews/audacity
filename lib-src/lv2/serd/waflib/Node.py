...
class Node(object):
    ...
    def mkdir(self):
        ...
        if self.name:
            ...
            if not os.path.isdir(self.abspath()):
                raise Errors.WafError(
                    "Could not create the directory %s" % self.abspath()
                )
            try:
                self.children
    ...
    def ant_iter(
        self, accept=None, maxdepth=25, pats=[], dir=False, src=True, remove=True
    ):
    ...
    def ant_glob(self, *k, **kw):
        ...
        ret = [
            x
            for x in self.ant_iter(
                accept=accept,
                pats=[to_pat(incl), to_pat(excl)],
