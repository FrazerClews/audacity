...
exts_tex = [".ltx", ".tex"]
re_tex = re.compile(
    r"\\(?P<type>include|bibliography|putbib|includegraphics|input|import|bringin|lstinputlisting)(\[[^\[\]]*\])?{(?P<file>[^{}]*)}",
    re.M,
)
...
def configure(self):
    ...
    for (
        ...
        "tex latex pdflatex xelatex bibtex dvips dvipdf ps2pdf makeindex pdf2ps".split()
    ):
        try:
