...
def main():
    ...
    if wav_in.nchannels != n_audio_in:
        print(
            "Input has %d channels, but plugin has %d audio inputs\n"
        ...
        if port.is_a(lv2_InputPort):
            if port.is_a(lv2_AudioPort):
                audio_input_buffers.append(
                    numpy.array(channels[len(audio_input_buffers)], numpy.float32)
                )
                ...
                default = lilv.lilv_node_as_float(
                    lilv.lilv_nodes_get_first(port.get_value(lv2_default))
                )
            ...
            if port.is_a(lv2_AudioPort):
                audio_output_buffers.append(
                    numpy.array([0] * wav_in.nframes, numpy.float32)
                )
    ...
    for chunk in numpy.array_split(data, CHUNK_SIZE):
        wav_out.writeframes(
            wave.struct.pack("%u%s" % (len(chunk), wav_in.struct_fmt_code), *chunk)
        )
    wav_out.close()
