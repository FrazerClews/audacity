...
def find_qt4_binaries(self):
    ...
    qtbin = (
        self.cmd_and_log([self.env.QMAKE, "-query", "QT_INSTALL_BINS"]).strip() + os.sep
    )
...
def find_qt4_libraries(self):
    qtlibs = getattr(Options.options, "qtlibs", None) or os.environ.get(
        "QT4_LIBDIR", None
    )
    if not qtlibs:
        try:
    ...
    qtincludes = (
        os.environ.get("QT4_INCLUDES", None)
