...
def init_perlext(self):
    ...
    self.env["cshlib_PATTERN"] = self.env["cxxshlib_PATTERN"] = self.env[
        "perlext_PATTERN"
    ]
...
def check_perl_ext_devel(self):
    ...
    env["CFLAGS_PERLEXT"] = read_out(
        " -MConfig -e'print \"$Config{ccflags} $Config{cccdlflags}\"'"
    )
    env["XSUBPP"] = read_out(
        " -MConfig -e'print \"$Config{privlib}/ExtUtils/xsubpp$Config{exe_ext}\"'"
    )
