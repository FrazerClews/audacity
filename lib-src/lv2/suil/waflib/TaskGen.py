...
class task_gen(object):
    ...
    def __init__(self, *k, **kw):
        ...
        if not "bld" in kw:
            ...
            try:
                self.idx = self.bld.idx[id(self.path)] = (
                    self.bld.idx.get(id(self.path), 0) + 1
                )
...
class subst_pc(Task.Task):
    def run(self):
        ...
        if getattr(self.generator, "subst_fun", None):
            ...
            if code is not None:
                self.outputs[0].write(
                    code, encoding=getattr(self.generator, "encoding", "ISO8859-1")
                )
            return
        code = code.replace("%", "%%")
        lst = []
        ...
        try:
            ...
            for x in lst:
                tmp = (
                    getattr(self.generator, x, "")
                    or self.env.get_flat(x)
                    or self.env.get_flat(x.upper())
                )
                d[x] = str(tmp)
        code = code % d
        self.outputs[0].write(
...
def add_pcfile(self, node):
    ...
    self.bld.install_files(
        getattr(self, "install_path", "${LIBDIR}/pkgconfig/"), tsk.outputs
    )
