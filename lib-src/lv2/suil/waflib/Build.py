    ...
    def execute_build(self):
        ...
        try:
            ...
            if self.progress_bar == 1:
                ...
                self.to_log(
                    self.progress_line(c, c, Logs.colors.BLUE, Logs.colors.NORMAL)
                )
                print("")
                sys.stdout.flush()
        ...
        if self.targets == "*":
            ...
            if ln.is_child_of(self.bldnode):
                ...
                Logs.warn(
                    ...
                    % (ln.abspath(), self.srcnode.abspath())
                )
                ln = self.srcnode
            for tg in self.groups[self.cur]:
                try:
        ...
        for x in self.source:
            if isinstance(x, waflib.Node.Node):
                ...
                if not y:
                    if Logs.verbose:
                        Logs.warn(
                            "Could not find %s immediately (may cause broken builds)"
                            % x
                        )
                    ...
                    for tg in self.generator.bld.groups[idx]:
                        ...
                        raise Errors.WafError(
                            "Could not find %r in %r" % (x, self.path)
                        )
            buf.append(y)
        self.inputs = buf
