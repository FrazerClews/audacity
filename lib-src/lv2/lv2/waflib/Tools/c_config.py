...
def exec_cfg(self, kw):
    ...
    if "modversion" in kw:
        version = self.cmd_and_log(
            [kw["path"], "--modversion", kw["modversion"]]
        ).strip()
        self.define(
            "%s_VERSION"
...
def validate_c(self, kw):
    ...
    if "function_name" in kw:
        ...
        if "field_name" in kw:
            field = kw["field_name"]
            kw["code"] = to_header(kw) + SNIP_FIELD % {
                "type_name": tu,
                "field_name": field,
            }
            if not "msg" in kw:
...
def post_check(self, *k, **kw):
    ...
    if "define_name" in kw:
        if (
            ...
            if (
                kw["execute"]
                and kw.get("define_ret", None)
                and isinstance(is_success, str)
            ):
...
def get_suncc_version(conf, cc):
    ...
    version_re = re.compile(
        r"cc:\s+sun\s+(c\+\+|c)\s+(?P<major>\d*)\.(?P<minor>\d*)", re.I
    ).search
...
def multicheck(self, *k, **kw):
    ...
    for x in tasks:
        if x.hasrun != Task.SUCCESS:
            ...
            self.fatal(
                kw.get("fatalmsg", None)
                or "One of the tests has failed, see the config.log for more information"
            )
    self.end_msg("ok")
