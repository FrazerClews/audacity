...
def waf_entry_point(current_directory, version, wafdir):
    ...
    if not Context.run_dir:
        if "-h" in sys.argv or "--help" in sys.argv:
            ...
            sys.exit(0)
        Logs.error(
            "Waf: Run from a directory containing a file named %r"
...
class Dist(Context.Context):
    ...
    def get_excl(self):
        try:
            ...
            self.excl = (
                Node.exclude_regs
                + " **/waf-1.7.* **/.waf-1.7* **/waf3-1.7.* **/.waf3-1.7* **/*~ **/*.rej **/*.orig **/*.pyc **/*.pyo **/*.bak **/*.swp **/.lock-w*"
            )
...
def update(ctx):
    ...
    if not lst:
        lst = [
            x
            for x in Utils.listdir(Context.waf_dir + "/waflib/extras")
            if x.endswith(".py")
        ]
    for x in lst:
        tool = x.replace(".py", "")
