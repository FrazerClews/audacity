...
def set_options(opt, debug_by_default=False):
    ...
    dirs_options.add_option(
        "--bindir", type="string", help="Executable programs [Default: PREFIX/bin]"
    )
    dirs_options.add_option(
        "--configdir", type="string", help="Configuration data [Default: PREFIX/etc]"
    )
    dirs_options.add_option(
        "--datadir", type="string", help="Shared data [Default: PREFIX/share]"
    )
    dirs_options.add_option(
        "--includedir", type="string", help="Header files [Default: PREFIX/include]"
    )
    dirs_options.add_option(
        "--libdir", type="string", help="Libraries [Default: PREFIX/lib]"
    )
    dirs_options.add_option(
        "--mandir", type="string", help="Manual pages [Default: DATADIR/man]"
    )
    dirs_options.add_option(
        "--docdir", type="string", help="HTML documentation [Default: DATADIR/doc]"
    )
    if debug_by_default:
        opt.add_option(
            "--optimize",
    ...
    dirs_options.add_option(
        "--lv2dir", type="string", help="LV2 bundles [Default: LIBDIR/lv2]"
    )
    g_step = 1
...
def check_header(conf, lang, name, define="", mandatory=True):
    ...
    if define != "":
        check_func(
            header_name=name, includes=includes, define_name=define, mandatory=mandatory
        )
    else:
...
def nameify(name):
    return (
        name.replace("/", "_").replace("++", "PP").replace("-", "_").replace(".", "_")
    )
    ...
    if check:
        ...
        if conf.env.PARDEBUG:
            ...
            found = conf.check_cfg(
                package=pkg_name + "D", args="--cflags --libs", **args
            )
            if found:
                pkg_name += "D"
        if mandatory:
...
def configure(conf):
    ...
    conf.env["PREFIX"] = normpath(
        os.path.abspath(os.path.expanduser(conf.env["PREFIX"]))
    )
    ...
    if Options.options.lv2dir:
        ...
        if sys.platform == "darwin":
            conf.env["LV2DIR"] = os.path.join(
                os.getenv("HOME"), "Library/Audio/Plug-Ins/LV2"
            )
    ...
    if Options.options.strict:
        ...
        conf.env.append_value(
            "CXXFLAGS", ["-ansi", "-Wnon-virtual-dtor", "-Woverloaded-virtual"]
        )
        append_cxx_flags(
            [
                "-Wall",
...
def build_dox(bld, name, version, srcdir, blddir, outdir="", versioned=True):
    ...
    for i in range(1, 8):
        bld.install_files(
            "${MANDIR}/man%d" % i,
            bld.path.get_bld().ant_glob("doc/man/man%d/*" % i, excl="**/_*"),
        )
...
def pre_test(ctx, appname, dirs=["src"]):
    ...
    try:
        try:
            subprocess.call(
                ("lcov %s -z" % diropts).split(), stdout=clear_log, stderr=clear_log
            )
        except:
    ...
    try:
        try:
            ...
            if g_is_child:
                base = ".."
            subprocess.call(
                ("lcov -c %s -b %s" % (diropts, base)).split(),
                stdout=coverage_lcov,
                stderr=coverage_log,
            )
            subprocess.call(
                ["lcov", "--remove", "coverage.lcov"] + remove,
                stdout=coverage_stripped_lcov,
                stderr=coverage_log,
            )
        ...
        print("")
        Logs.pprint(
            "GREEN", "Waf: Leaving directory `%s'" % os.path.abspath(os.getcwd())
        )
        top_level = len(ctx.stack_path) > 1
        if top_level:
...
def run_tests(
    ctx, appname, tests, desired_status=0, dirs=["src"], name="*", headers=False
):
    failures = 0
    diropts = ""
    for i in dirs:
