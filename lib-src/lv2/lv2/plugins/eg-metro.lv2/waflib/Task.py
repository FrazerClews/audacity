...
class TaskBase(evil):
    ...
    def attr(self, att, default=None):
        ...
        if ret is self:
            return getattr(self.__class__, att, default)
        return ret
    ...
    def colon(self, var1, var2):
        ...
        if isinstance(tmp, str):
            ...
            if Logs.verbose and not tmp and it:
                Logs.warn(
                    "Missing env variable %r for task %r (generator %r)"
                    % (var1, self, self.generator)
                )
            lst = []
            for y in it:
                lst.extend(tmp)
...
class Task(TaskBase):
    ...
    def runnable_status(self):
        ...
        for node in self.outputs:
            try:
                ...
                Logs.debug(
                    "task: task %r must run as the output nodes do not exist" % self
                )
                return RUN_ME
        if new_sig != prev_sig:
            return RUN_ME
        ...
        for x in self.inputs + self.dep_nodes:
            try:
                ...
                raise Errors.WafError(
                    "Missing node signature for %r (required by %r)" % (x, self)
                )
        if bld.deps_man:
    ...
    def sig_implicit_deps(self):
        ...
        if Logs.verbose:
            Logs.debug(
                "deps: scanner for %s returned %s %s"
            ...
            if Logs.verbose:
                for k in bld.node_deps.get(self.uid(), []):
                    try:
                        ...
                        Logs.warn(
                            "Missing signature for node %r (may cause rebuilds)" % k
                        )
        else:
            return sig
    ...
    def put_files_cache(self):
        ...
        tmpdir = tempfile.mkdtemp(
            prefix=self.generator.bld.cache_global + os.sep + "waf"
        )
        try:
            shutil.rmtree(dname)
        except Exception:
    ...
    for (var, meth) in extr:
        if var == "SRC":
            if meth:
                ...
                app('" ".join([a.path_from(bld.bldnode) for a in tsk.inputs])')
        elif var == "TGT":
            if meth:
                app("tsk.outputs%s" % meth)
            else:
                app('" ".join([a.path_from(bld.bldnode) for a in tsk.outputs])')
        elif meth:
            if meth.startswith(":"):
                m = meth[1:]
                if m == "SRC":
    ...
    for x in range(len(extr)):
        ...
        if var == "SRC":
            if meth:
                ...
                app("lst.extend([a.path_from(bld.bldnode) for a in tsk.inputs])")
        elif var == "TGT":
            if meth:
                app("lst.append(tsk.outputs%s)" % meth)
            else:
                app("lst.extend([a.path_from(bld.bldnode) for a in tsk.outputs])")
        elif meth:
            if meth.startswith(":"):
                m = meth[1:]
                if m == "SRC":
