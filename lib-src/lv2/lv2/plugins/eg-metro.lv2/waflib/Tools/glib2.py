...
def add_settings_enums(self, namespace, filename_list):
    if hasattr(self, "settings_enum_namespace"):
        raise Errors.WafError(
            "Tried to add gsettings enums to '%s' more than once" % self.name
        )
...
def process_settings(self):
    ...
    def compile_schemas_callback(bld):
        ...
        command = Utils.subst_vars(
            "${GLIB_COMPILE_SCHEMAS} ${GSETTINGSSCHEMADIR}", bld.env
        )
...
def configure(conf):
    ...
    conf.find_program(
        "glib-compile-schemas", var="GLIB_COMPILE_SCHEMAS", mandatory=False
    )
