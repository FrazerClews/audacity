...
def apply_link(self):
    ...
    if inst_to:
        self.install_task = self.bld.install_files(
            inst_to, self.link_task.outputs[:], env=self.env, chmod=self.link_task.chmod
        )
...
def apply_implib(self):
    ...
    if getattr(self, "defs", None) and self.env.DEST_BINFMT == "pe":
        ...
        if "msvc" in (self.env.CC_NAME, self.env.CXX_NAME):
            self.env.append_value(
                "LINKFLAGS", "/def:%s" % node.path_from(self.bld.bldnode)
            )
    ...
    if not inst_to:
        return
    self.implib_install_task = self.bld.install_as(
        "${LIBDIR}/%s" % implib.name, implib, self.env
    )
...
def apply_vnum(self):
    ...
    if getattr(self, "install_task", None):
        ...
        if self.env.DEST_OS == "openbsd":
            ...
            t1 = bld.install_as(
                path + os.sep + name3, node, env=self.env, chmod=self.link_task.chmod
            )
